...
#switch_list={'cc:37:ab:6b:0d:a6':['spine-2', '0x0000000000000012'], 'cc:37:ab:6e:e4:c6':['leaf-2', '0x0000000000000022']}
switch_list={'cc:37:ab:7c:be:68':['spine-1', '0x0000000000000011'], 'cc:37:ab:7c:bf:ee':['spine-2', '0x0000000000000012'], 'cc:37:ab:7c:b9:d6' : ['leaf-1', '0x0000000000000021'], 'cc:37:ab:7c:ba:da':['leaf-2', '0x0000000000000022'], 'cc:37:ab:17:7b:c0':['fabric-1', '0x000000000000fab1'], '70:72:cf:f5:60:9e':['fabric-2', '0x000000000000fab2']}
...
def harvest(options):
    ...
    if options.verify:
        ...
        while count != len(report_dataset):
            ...
            if resp['verified']:
                ...
                if any(resp['lease']['hardware'] in s for s in switch_list):
    ...
    for lease in verified:
        name = lease['client-hostname']
        if ifilter != None and name in ifilter or matched(rfilter, name):
            out.write(format(name, '<'+str(size)) + ' IN A ' + lease['ip_address'] + ' MAC: ' +lease['hardware'] + '\n')
    if options.dest != '-':
        out.close()
    return count
